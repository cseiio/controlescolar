    ...
    foreach($datos as $dato){
        ...
        if($dato->tipo_operacion=="UN_PLANTEL"){
            if($dato->grupo=="TODOS_LOS_GRUPOS"){
                ...
                $grupos=array (0=>(object)array("id_grupo"=>$dato->grupo));
            }
            ...
            foreach($grupos as $grupo){
                if($dato->materia=="TODAS_LAS_MATERIAS"){
                    $materias =$this->M_grupo->get_materias_grupo($grupo->id_grupo);  
                }
                else{
                    $materias=array (0=>(object)array("clave"=>$dato->materia));
                }
